...
Do not use names such as `references-unverified-source` or `references-nonexistent-artifact` unless the schema actually contains a resolvable reference relation and can reject the fixture at Layer 1 for that reason.
...
- enum values;
- field shape;
- decomposition metadata required by declared decomposition type;
- explicit non-claims;
...
`ArtifactSourceLock` does not currently carry a schema-level `verification_status` field or a graph-walkable lock-chain relation. Therefore `references-unverified-source`, `references-nonexistent-artifact`, content-hash-mismatch, and circular-reference fixtures are out of scope for Layer 1 in this PR.
