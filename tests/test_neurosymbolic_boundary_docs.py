...
def test_method_families_are_bound_in_role_matrix() -> None:
    text = read(ROLE_MATRIX)
    for family in REQUIRED_METHOD_FAMILIES:
        assert family in text


def test_failure_modes_are_named() -> None:
    text = read(FAILURE_MODES) + read(POSITION) + read(EXAMPLE_TASK)
...
def test_no_local_authority_claims_for_neurosymbolic_methods() -> None:
    ...
    forbidden_claims = [
        ...
        "superconscious may execute a symbolic policy",
