...
Superconscious will generalize the current Gemma / Gemma Scope steering pilot into a provider-agnostic interpretability harness.
...
A registry-only provider exposes external interpretability artifacts such as feature dashboards, registry entries, or public circuit graphs. It can supply evidence refs, but cannot by itself prove runtime behavior unless paired with replayable model execution.
...
- black-box provider claims `hidden_states`;
