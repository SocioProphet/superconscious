...
**Status:** H6 replayed doctrine; falsification planning surface, not runtime evidence.  
**Origin branch:** `doctrine/architecture-falsification-v0-1`.  
**Audit disposition:** replay-after-demotion from branch reconciliation audit.  
**Scope:** Superconscious-facing falsification doctrine for the artifact / evidence / gate architecture.

## H6 / H4 boundary

This document is retained because it names useful failure observables for the architecture. It does not certify that those observables have been tested, realized, resolved, or instrumented.

This document does **not** claim:

```text
runtime gate enforcement implemented
Atlas / Gatekeeper integration complete
SHACL coverage complete
certificate semantics verified at runtime
model artifacts safe or unsafe by default
architecture falsified
architecture validated
runtime adapter authority granted
```

Superconscious does not become the authority for the schemas, gates, certificates, runtime decisions, or downstream component behavior described here. Superconscious may keep the falsification doctrine visible in the governed cognition loop and may emit traces that reference this doctrine.

## Purpose

This document names observations that would force structural revision of the three-layer architecture:

```text
artifact layer -> evidence layer -> gate layer -> runtime ecosystem
```

The architecture composes by reference: artifact-producing systems produce artifacts; certificate programs consume artifacts into evidence; gate systems consume evidence into promotion decisions; downstream runtime components must respect the gate decision.

The guarded failure mode is not imperfect gate prediction. The guarded failure mode is stricter: one layer cannot, in principle, produce what the next layer needs to decide. That is structural inadequacy, and this document names it in advance.
...
| Methodology | This document | Architecture governance | Are the falsification claims empirically testable and kept current? |
...
The origin branch also modified `ARCHITECTURE.md` and `ROADMAP.md`. Those edits are intentionally not replayed in this PR. This PR captures the new standalone doctrine file first. Any navigation or roadmap updates should happen separately after this doctrine lands and can cite the hardened file.